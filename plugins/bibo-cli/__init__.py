"""Komendy Bibo w CLI Hermesa.
    ...
    hermes bibo update    — pobiera i uruchamia update.sh z GitHuba (zawsze najnowszy)
...
def _aktualizuj(args) -> int:
    ...
    print(f"Aktualizuję Bibo z {url}\n", flush=True)
    return subprocess.call(["bash", "-c", f'set -o pipefail; curl -fsSL "{url}" | bash'], env=env)
